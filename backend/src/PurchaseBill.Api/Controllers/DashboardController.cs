...
public class DashboardController(IDashboardService dashboardService) : ControllerBase
{
    ...
    public async Task<ActionResult<IReadOnlyList<LatestOrderDto>>> GetLatestOrders(
        [FromQuery] DashboardRange range = DashboardRange.Today, CancellationToken ct = default) =>
        Ok(await dashboardService.GetLatestOrdersAsync(range, ct));
    ...
    public async Task<ActionResult<IReadOnlyList<OldestItemDto>>> GetOldestItems(
        [FromQuery] DashboardRange range = DashboardRange.Today, CancellationToken ct = default) =>
        Ok(await dashboardService.GetOldestItemsAsync(range, ct));
